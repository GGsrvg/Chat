...
class ChatViewModel : ViewModel() {
    ...
    private fun connectWebSocket() {
        var dispose = webSocketObservable()
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({
                messages.add(it)
            }, {
                Log.e("Observable", it.message)
            })
    }
    ...
    private fun webSocketObservable() : Observable<Message> {
        return Observable.create { subscriber ->
            ...
            webSocket = client.newWebSocket(request, getWebSocketListener(subscriber))
        }
    }

    private fun getWebSocketListener(observableEmitter: ObservableEmitter<Message>): WebSocketListener {
        return object : WebSocketListener() {
            ...
            override fun onMessage(webSocket: WebSocket?, text: String?) {
                ...
                observableEmitter.onNext(Message(text!!, false))
            }

            override fun onMessage(webSocket: WebSocket?, bytes: ByteString?) {
                Log.e(TAG, "onMessageBytes ${bytes!!.hex()}")
                observableEmitter.onNext(Message(bytes!!.hex(), false))
            }
    ...
    companion object{
        ...
        private const val localserver = "wss://192.168.0.102:5001/ws"
    }
}
